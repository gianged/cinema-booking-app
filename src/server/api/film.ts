...
router.post("/film", (req, res) => {
  ...
  const query =
    "INSERT INTO film(id, filmName, filmDescription, poster, backdrop, premiere, filmCategory, isActive) SELECT MAX(id) + 1, ?, ?, ?, ?, ?, ?, true FROM film";
  connection.query(
    query,
    [filmName, filmDescription, poster, backdrop, premiere, filmCategory],
    (err, results) => {
      if (err) {
        console.log(err);
        res.sendStatus(500).send("Server-side error");
...
router.put("/film/:id", (req, res) => {
  ...
  const { id } = req.params;
  const query =
    "UPDATE film SET filmName = ?, filmDescription = ?, poster = ?, backdrop = ?, premiere = ?, filmCategory = ?, isActive = ? WHERE id = ?";
  connection.query(
    query,
    [
      filmName,
